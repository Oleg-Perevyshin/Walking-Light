...
const DeviceList = ({ devices, selectedDevice, onChange }) => (
  <Select
    className="flex-elem fe1 select small"
    style={`border: 1px solid #${devices[selectedDevice.da].dc}`}
    onChange={onChange}
    value={selectedDevice.da}
  >
...
const DeviceAPI = ({
  ...
  return (
    <div className="flex-cont">
      <Select
        className="flex-elem fe4 select small"
      ...
      >
        {api.map((_, i) => (
          <option value={i}>
            {translate(dictionary, `api.${type}.${i}.name`)}
          </option>
        ))}
      </Select>
      <Select
        className="flex-elem fe5 select small"
        style={`border: 1px solid #${color}`}
        ...
        {new Array(api[selectedCategory]).fill(null).map((_, i) => {
          return (
            <option value={i}>
              {translate(
                dictionary,
                `api.${type}.${selectedCategory}.options.${i}`
              )}
            </option>
          );
        })}
      ...
      <Button
        className="flex-elem fe6 button e2 gray small"
        onClick={deletable && onRemoveButtonClick}
      >
        -
      </Button>
      <Button
        className="flex-elem fe7 button e2 blue small"
...
const loadDeviceData = (ip) => {
  return fetchJSON(`//${ip}/config.json`)
    .then(function (json) {
      const device = { ip: ip, status: 'loading', ...json };
      return fetchJSON(`//${ip}/mcu/api`).then((api) => {
        device.api = api;
        return fetchJSON(`//${ip}/lang?dl=${json.dl}.json`).then((dl) => {
...
class Scripts extends Component {
  ...
  createScript() {
    ...
    scripts.push({
      collapsed: false,
      name: `${translate(this.context.dictionary, 'sc.m.new')} (${
        scripts.length
      })`,
      enabled: false,
      custom_param: '',
  ...
  saveScripts() {
    ...
    fetch('/mcu/scw', {
      method: 'POST',
      body: JSON.stringify(
        scripts.map(({ name, enabled, custom_param, events, actions }, id) => ({
          id,
          name,
          enabled,
          custom_param,
  ...
  onCustomParamInput(index, event) {
    const { scripts } = { ...this.state };
    scripts[index].custom_param = event.target.value
      .toUpperCase()
      .replace(/[^0-9]/g, '')
      .substring(0, 4);
    this.setState({ scripts });
  ...
  addDevice(category, scriptIndex, after) {
    ...
    if (target) {
      ...
      this.setState({ scripts });
    } else console.error('Error Adding Device');
  }

  removeDevice(category, scriptIndex, selectedDeviceIndex) {
    const { scripts } = { ...this.state };
    let target;
    if (category === EVENTS) target = scripts[scriptIndex].events;
    else if (category === ACTIONS) target = scripts[scriptIndex].actions;
    if (target) {
      target.splice(selectedDeviceIndex, 1);
      this.setState({ scripts });
    } else console.error('Error Removing Device');
  }
  ...
  onDeviceChange(category, scriptIndex, selectedDeviceIndex, e) {
    ...
    let target =
      category === EVENTS
        ? scripts[scriptIndex].events
        : scripts[scriptIndex].actions;
    let data = [0, 0];
    if (target) {
      ...
      this.setState({ scripts });
    } else console.error('Error Changing Device');
  }
  ...
  onCategoryChange(
    ...
    if (target) {
      ...
      this.setState({ scripts });
    } else console.error('Error Changing Category');
  }
  ...
  onSubcategoryChange(
    ...
    if (target) {
      ...
      this.setState({ scripts });
    } else console.error('Error Changing Subcategory');
  }
  ...
  onAddButtonClick(category, scriptIndex, selectedDeviceIndex, after) {
    ...
    if (target) {
      target.splice(++after, 0, data);
      this.setState({ scripts });
    } else console.error('Error Adding');
  }
  ...
  onRemoveButtonClick(
    ...
    if (target) {
      ...
      this.setState({ scripts });
    } else console.error('Error Removing');
  }
  ...
  render() {
    ...
    return this.state.loading ? (
      ...
      <div>
        <div>
          <Button
            className="button w70 grey large"
            onClick={::this.createScript}
          >
            <Message key="sc.m.create" />
          </Button>
        </div>
        <div>
          <Button
            className="button w70 red large"
            onClick={::this.removeScripts}
          >
            <Message key="sc.m.clear" />
          </Button>
        </div>
        {scripts &&
          scripts.map((script, scriptIndex) => (
            <div className="sb">
              <div className="sc-main">
                <input
                  type="checkbox"
                  className="switch-hor"
                  id={'switch-hor' + scriptIndex}
                ...
                <Input
                  className="sc-name w75"
                  type="text"
                  value={script.name}
                  maxlength="16"
                  onInput={this.onNameInput.bind(this, scriptIndex)}
                >
                  {script.name}
                </Input>
                ...
                <div className="spl">
                  <h3 className="bold tsc">
                    <Message key="sc.m.events" />
                  </h3>
                  <div>
                    {script.events.map((selectedDevice, selectedDeviceIndex) =>
                      devices.hasOwnProperty(selectedDevice.da) &&
                      devices[selectedDevice.da].status === 'ok' ? (
                        <div className="sc-device">
                          <div className="flex-cont">
                            <DeviceList
                              devices={devices}
                              selectedDevice={selectedDevice}
                              onChange={this.onDeviceChange.bind(
                                this,
                                EVENTS,
                                scriptIndex,
                                selectedDeviceIndex
                            ...
                            />
                            <Button
                              className="flex-elem fe2 button e2 gray small"
                              onClick={
                                script.events.length > 1 &&
                                this.removeDevice.bind(
                                  this,
                                  EVENTS,
                                  scriptIndex,
                                  selectedDeviceIndex
                            ...
                            <Button
                              className="flex-elem fe3 button e2 blue small"
                              onClick={this.addDevice.bind(
                                this,
                                EVENTS,
                                scriptIndex,
                                selectedDeviceIndex
                          ...
                          {selectedDevice.params.map((selected, index) => {
                            ...
                            return (
                              <DeviceAPI
                                ...
                                onCategoryChange={this.onCategoryChange.bind(
                                  this,
                                  EVENTS,
                                  scriptIndex,
                                  selectedDeviceIndex,
                                ...
                                onSubcategoryChange={this.onSubcategoryChange.bind(
                                  this,
                                  EVENTS,
                                  scriptIndex,
                                  selectedDeviceIndex,
                                ...
                                onAddButtonClick={this.onAddButtonClick.bind(
                                  this,
                                  EVENTS,
                                  scriptIndex,
                                  selectedDeviceIndex,
                                ...
                                onRemoveButtonClick={this.onRemoveButtonClick.bind(
                                  this,
                                  EVENTS,
                                  scriptIndex,
                                  selectedDeviceIndex,
        ...
        <Button className="button w70 green large" onClick={::this.saveScripts}>
          <Message key="a.save" />
        </Button>
      </div>
    );
