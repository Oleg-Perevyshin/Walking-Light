...
void configLoad() {
  ...
  strncpy(da, root["da"].as<char*>(), sizeof(da) - 1);
  strncpy(rch, root["rch"].as<char*>(), sizeof(rch) - 1);
  strncpy(dn, root["dn"].as<char*>(), sizeof(dn) - 1);
  strncpy(dc, root["dc"].as<char*>(), sizeof(dc) - 1);
  strncpy(dl, root["dl"].as<char*>(), sizeof(dl) - 1);
  strncpy(dls, root["dls"].as<char*>(), sizeof(dls) - 1);
  dz = root["dz"].as<int>();
  strncpy(dv, root["dv"].as<char*>(), sizeof(dv) - 1);
  strncpy(du, root["du"].as<char*>(), sizeof(du) - 1);
  strncpy(dp, root["dp"].as<char*>(), sizeof(dp) - 1);
  strncpy(staSSID, root["staSSID"].as<char*>(), sizeof(staSSID) - 1);
  strncpy(staPSK, root["staPSK"].as<char*>(), sizeof(staPSK) - 1);
  staIPMode = root["staIPMode"].as<int>();
  strncpy(staIP, root["staIP"].as<char*>(), sizeof(staIP) - 1);
  strncpy(staMS, root["staMS"].as<char*>(), sizeof(staMS) - 1);
  strncpy(staGW, root["staGW"].as<char*>(), sizeof(staGW) - 1);
  strncpy(apSSID, root["apSSID"].as<char*>(), sizeof(apSSID) - 1);
  strncpy(apPSK, root["apPSK"].as<char*>(), sizeof(apPSK) - 1);
  strncpy(apIP, root["apIP"].as<char*>(), sizeof(apIP) - 1);
  strncpy(apMS, root["apMS"].as<char*>(), sizeof(apMS) - 1);
  strncpy(apGW, root["apGW"].as<char*>(), sizeof(apGW) - 1);
  f.close();
  configBuffer.clear();
}
...
void configLoadDefaults() {
  ...
  strncpy(staSSID, "SOI-GW", sizeof(staSSID) - 1);
  strncpy(staPSK, "MCMega20051983!", sizeof(staPSK) - 1);
  staIPMode = IP_DHCP;
